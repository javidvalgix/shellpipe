import { FolderOpen, Terminal, X, Plus } from "lucide-react";
import useTabStore from "@/stores/tab.store";
import PathBar from "./path-bar";
import ListView from "./file-viewer/list-view";
import TerminalView from "./file-viewer/terminal-view";
import SessionPickerView from "./file-viewer/session-picker-view";
...
export default function FileBrowser() {
    ...
    const handleTerminalTabCreate = () => {
        // Create a terminal tab without a session; the tab itself will prompt session selection
        const tabId = useTabStore.getState().createTerminalTab();
        setActiveTab(tabId);
    };
    ...
    return (
        <>
            <div className="flex flex-col h-full border rounded-xl">
                            ...
                            <div className="flex flex-nowrap h-full items-center">
                                ...
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="start">
                                        <DropdownMenuItem onClick={handleTabCreate}>
                                            <FolderOpen size={14} className="mr-2" />
                                            SFTP Browser
                                        </DropdownMenuItem>
                                        <DropdownMenuItem onClick={handleTerminalTabCreate}>
                ...
                {/* PathBar */}
                {tabs.length > 0 && activeTab?.type !== "terminal" && <PathBar />}

                {/* Bookmarks */}
                {showBookmarks && activeTab?.type !== "terminal" && (
                    <Bookmarks />
                )}
                        ...
                        {tabs.map(tab => (
                            ...
                            >
                                {tab.type === "terminal" ? (
                                    ...
                                    <SessionPickerView tabId={tab.id} />
                                )}
                            </div>
                        ))}
                    </div>
                ...
                {tabs.length === 0 && (
                    <div className="relative flex flex-1 w-full flex-col items-center justify-center overflow-hidden bg-background rounded-xl">
                        <MorphingText texts={["Shellpipe", "sFTP Browser", "SSH"]} />
                    </div>
                )}
            </div>
