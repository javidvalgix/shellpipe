...
interface SessionStore {
  ...
  moveSessionToWorkspace: (sessionId: string, workspaceId: string | undefined) => void;
  ...
  getDecryptedConnectionConfig: (sessionId: string) => Promise<{
    host: string;
    port: number;
    username: string;
    password?: string;
...
async function getKey(): Promise<CryptoKey> {
  ...
  return crypto.subtle.deriveKey(
    {
      name: "PBKDF2",
      salt: stringToArrayBuffer("shellpipe-38c11fc4-cf80-4d8a-a9fd-ececc07c704f"), // In production, use a random salt
      iterations: 100000,
      hash: "SHA-256",
    },
    keyMaterial,
    ...
    persist(
      immer((set, get) => ({
        ...
        addSession: async (sessionData) => {
          ...
          // Add to top-level sidebar order only if not inside a workspace
          if (!newSession.workspaceId) {
            useSidebarStore.getState().addItem(newSession.id);
          }
          ...
          set((state) => {
            ...
            if (sessionIndex !== -1) {
              const updatedSession = {
                ...
                updatedAt: new Date(),
                workspaceId: state.sessions[sessionIndex].workspaceId,
              } as ISession;
        ...
        moveSessionToWorkspace: (sessionId, workspaceId) => {
          ...
          // Maintain top-level sidebar order
          if (workspaceId) {
            useSidebarStore.getState().removeItem(sessionId);
          } else {
            useSidebarStore.getState().addItem(sessionId);
          }
        },

        reorderSessions: (orderedIds: string[]) => {
          set((state) => {
            // Find the positions of these IDs in the current sessions array
            const positions = orderedIds
              .map((id) => state.sessions.findIndex((s) => s.id === id))
              .filter((pos) => pos !== -1);
            ...
            sortedPositions.forEach((pos, i) => {
              state.sessions[pos] = reorderedSessions[i];
            });
          });
        },
        ...
        disconnectSession: async (sessionId) => {
          ...
          if (!connectionState?.connectionId) {
            // No SFTP connection but still mark as disconnected (e.g. terminal-only sessions)
            get().updateSessionStatus(sessionId, "disconnected");
            return;
          }
        ...
        getDecryptedConnectionConfig: async (sessionId) => {
          const session = get().getSessionById(sessionId);
