...
import { ArrowLeft, ArrowRight, ClipboardIcon, Download, EllipsisIcon, FilePlus, FolderPlus, FolderUp, Loader2, RefreshCcw, Star, UploadCloud } from "lucide-react";
...
import EditFileDialog from "@/dialogs/edit-file.dialog";
...
export default function PathBar() {
    ...
    const [newFileItem, setNewFileItem] = useState<FileItem | null>(null);
    ...
    const handleNewFile = () => {
        if (!tab?.filePath) {
            toast.error("No current directory");
            return;
        }
        const dir = tab.filePath.endsWith("/") ? tab.filePath : tab.filePath + "/";
        ...
        <div className="flex items-center gap-2 p-2 bg-background border-b border-t h-12">
            ...
            <div className="inline-flex -space-x-px rounded-md shadow-xs rtl:space-x-reverse">
                    ...
                    <DropdownMenuContent>
                        <DropdownMenuGroup>
                            <DropdownMenuItem onClick={handleNewFile}>
                                <FilePlus size={16} className="opacity-60" aria-hidden="true" />
                                <span>New File</span>
                            </DropdownMenuItem>
                            <DropdownMenuItem
                                onClick={() => {
                ...
                <Button
                    ...
                    disabled={!session || session.status !== 'connected'}
                    onClick={() => setCreateBookmarkDialogOpen(true)}
                >
            ...
            {tabId && (
                <EditFileDialog
                    ...
                    isNew={true}
                    onSaved={() => {
                        if (tabId && tab?.session?.id && tab?.filePath) {
