...
interface TabStore {
  ...
  createTerminalTab: (sessionId?: string) => string;
      ...
      immer((set, get) => ({
        ...
        createTerminalTab: (sessionId?) => {
          ...
          set((state) => {
            const newTab: ITab = {
              ...
              session,
              type: "terminal",
              title: session ? `Terminal: ${session.name}` : "Terminal",
              icon: "terminal",
              isActive: false,
              isPinned: false,
              isDirty: false,
        ...
        closeTab: (tabId) => {
          ...
          set((state) => {
            ...
            closedSessionId = tab.session?.id;
          ...
          // Auto-disconnect: if no remaining tabs reference this session
          if (closedSessionId) {
            const remaining = get().tabs.filter((t) => t.session?.id === closedSessionId);
            if (remaining.length === 0) {
              useSessionStore.getState().disconnectSession(closedSessionId);
            }
          }
        },
        ...
        closeAllTabs: () => {
          const affectedSessionIds = [
            ...new Set(
              get()
                .tabs.filter((t) => !t.isPinned && t.session?.id)
                .map((t) => t.session!.id)
            ),
          ];
          ...
          for (const sessionId of affectedSessionIds) {
            if (get().tabs.filter((t) => t.session?.id === sessionId).length === 0) {
        ...
        closeOtherTabs: (keepTabId) => {
          const affectedSessionIds = [
            ...new Set(
              get()
                .tabs.filter((t) => t.id !== keepTabId && !t.isPinned && t.session?.id)
                .map((t) => t.session!.id)
            ),
          ];
        ...
        closeTabsToRight: (fromTabId) => {
          const fromIndex = get().tabs.findIndex((t) => t.id === fromTabId);
          const affectedSessionIds =
            fromIndex !== -1
              ? [
                  ...new Set(
                    get()
                      .tabs.slice(fromIndex + 1)
                      .filter((t) => !t.isPinned && t.session?.id)
                      .map((t) => t.session!.id)
                  ),
                ]
              : [];
          ...
          set((state) => {
            const fi = state.tabs.findIndex((t) => t.id === fromTabId);
            if (fi === -1) return;

            const tabsToKeep = state.tabs.slice(0, fi + 1);
            const tabsToClose = state.tabs.slice(fi + 1);
